...
export default function CartDrawer({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    const handleUpdateLocalQty = (itemId: number, delta: number) => {
        ...
        const maxStock = currentItem.product_variant?.stock_quantity ?? 99;

        dirtyIds.current.add(itemId);
        setLocalItems(prev => prev.map(item =>
            item.id === itemId
                ? { ...item, quantity: Math.min(Math.max(1, item.quantity + delta), maxStock) }
                : item
        ));
    };
                    ...
                    <div className="flex-1 overflow-y-auto p-8 space-y-8">
                        {localItems.length === 0 ? (
                            ...
                            localItems.map((item: any) => {
                                // Guard: variant or product may be null if deleted mid-session
                                if (!item.product_variant?.product) {
                                    return (
                                        <div key={item.id} className="flex items-center justify-between py-4 border-b border-brand-surface">
                                            <p className="text-[9px] font-black uppercase tracking-widest text-brand-slate/40">
                                ...
                                const maxStock  = item.product_variant.stock_quantity ?? 99;
                                const atStockCap = item.quantity >= maxStock;
                                            ...
                                            <div className="flex justify-between items-end mt-3">
                                                ...
                                                <div className={`flex items-center border bg-brand-white transition-colors ${
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => handleUpdateLocalQty(item.id, 1)}
                                                        disabled={atStockCap}
                                                        className="px-3 py-1 text-xs font-bold hover:bg-brand-surface transition-colors disabled:opacity-20 disabled:cursor-not-allowed"
                                                    >
                                                        +
                                                    </button>
                                                </div>
