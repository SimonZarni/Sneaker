        ...
        >
            ...
            <div style={{ marginBottom: "24px" }}>
                <form onSubmit={(e) => { e.preventDefault(); apply({ search }); }} style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search product name..."
                        style={{ flex: 1, minWidth: 0, border: "1px solid #e5e7eb", backgroundColor: "#fff", padding: "12px 16px", fontSize: "12px", outline: "none" }} />
                    <button type="submit" style={{ padding: "12px 20px", backgroundColor: "#0A0A0A", color: "#fff", fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.15em", border: "none", cursor: "pointer", flexShrink: 0 }}>Search</button>
                </form>
                <div style={{ display: "flex", gap: "8px" }}>
                    <select value={activeBrand} onChange={(e) => apply({ brand: e.target.value })}
                        style={{ flex: 1, padding: "10px 12px", border: "1px solid #e5e7eb", backgroundColor: "#fff", fontSize: "11px", fontWeight: 700, outline: "none", cursor: "pointer" }}>
                        <option value="">All Brands</option>
                        {brands.map((b) => <option key={b.id} value={String(b.id)}>{b.name}</option>)}
                    </select>
                    {isFiltered && (
                        <button onClick={clearFilters} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "10px 14px", fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", border: "1px solid #fecaca", backgroundColor: "#fef2f2", color: "#dc2626", cursor: "pointer", flexShrink: 0 }}>
            ...
            <div style={{ backgroundColor: "#fff", border: "1px solid #f0f0f0" }}>
                ...
                {products.data.length === 0 && (
                    <div style={{ padding: "80px 20px", textAlign: "center" }}>
                        ...
                        <Link href={route("admin.products.create")} style={{ fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", color: "#0A0A0A", textDecoration: "none", borderBottom: "1px solid #0A0A0A", paddingBottom: "1px" }}>+ Add your first product</Link>
                    </div>
                )}
                        ...
                        <div className="admin-prod-mobile" style={{ padding: "14px 16px", display: "flex", gap: "12px", alignItems: "flex-start" }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: "8px", marginBottom: "2px" }}>
                                    <p style={{ fontSize: "12px", fontWeight: 900, textTransform: "uppercase", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{product.name}</p>
                                    <p style={{ fontSize: "12px", fontWeight: 900, flexShrink: 0 }}>${parseFloat(product.base_price).toFixed(2)}</p>
                                </div>
                                <p style={{ fontSize: "9px", color: "rgba(45,50,62,0.4)", marginBottom: "8px" }}>{product.brand} · {product.category} · {product.variants_count} SKUs</p>
                                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                                    <Link href={route("admin.products.edit", product.id)} style={{ fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", color: "#0A0A0A", textDecoration: "none", padding: "6px 12px", border: "1px solid #e5e7eb" }}>Edit</Link>
                                    <button onClick={() => handleToggle(product.id)} style={{ fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em", padding: "6px 12px", border: `1px solid ${product.is_active ? "#fde68a" : "#a7f3d0"}`, backgroundColor: product.is_active ? "#fffbeb" : "#ecfdf5", color: product.is_active ? "#b45309" : "#065f46", cursor: "pointer" }}>
                                        {product.is_active ? "Deactivate" : "Activate"}
                                    </button>
                                    <button onClick={() => handleDelete(product.id, product.name)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "32px", height: "32px", border: "1px solid #fecaca", backgroundColor: "#fef2f2", color: "#dc2626", cursor: "pointer" }}>
                        ...
                        <div className="admin-prod-desktop" style={{ display: "grid", gridTemplateColumns: "56px minmax(0,1.8fr) 110px 110px 80px 80px 160px", gap: "16px", alignItems: "center", padding: "14px 20px" }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                    ...
                                    {!product.is_active && <span style={{ flexShrink: 0, fontSize: "8px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", backgroundColor: "#fef2f2", color: "#dc2626", border: "1px solid #fecaca", padding: "1px 6px" }}>Inactive</span>}
                                </div>
                            ...
                            <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end", alignItems: "center" }}>
                                <Link href={route("admin.products.edit", product.id)} style={{ fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.1em", color: "#0A0A0A", textDecoration: "none", padding: "7px 14px", border: "1px solid #e5e7eb", whiteSpace: "nowrap" }}>Edit</Link>
                                <button onClick={() => handleToggle(product.id)} style={{ fontSize: "9px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em", padding: "7px 12px", border: `1px solid ${product.is_active ? "#fde68a" : "#a7f3d0"}`, backgroundColor: product.is_active ? "#fffbeb" : "#ecfdf5", color: product.is_active ? "#b45309" : "#065f46", cursor: "pointer", whiteSpace: "nowrap" }}>
                                    {product.is_active ? "Deactivate" : "Activate"}
                                </button>
                                <button onClick={() => handleDelete(product.id, product.name)} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "34px", height: "34px", border: "1px solid #fecaca", backgroundColor: "#fef2f2", color: "#dc2626", cursor: "pointer", flexShrink: 0 }}>
