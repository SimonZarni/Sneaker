...
export default function AdminInventoryIndex({ variants, stats, filters, admin }: Props) {
    ...
    return (
        <AdminLayout adminName={admin.name} active="inventory" pageTitle="Inventory" pageLabel="Stock Management">
            ...
            <div className="inv-stat-grid" style={{ gap: "12px", marginBottom: "24px" }}>
                {[
                    ...
                    <div key={card.label} style={{ backgroundColor: card.bg, padding: "20px 24px" }}>
                        <p style={{ fontSize: "9px", fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: "0.3em", color: card.sub, marginBottom: "8px" }}>
                            {card.label}
                        </p>
                        <p style={{ fontSize: "28px", fontWeight: 900, letterSpacing: "-0.05em", color: card.text, fontVariantNumeric: "tabular-nums" }}>
            ...
            <div style={{ backgroundColor: "#fff", border: "1px solid #f0f0f0", marginBottom: "2px" }}>
                <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column" as const, gap: "10px" }}>
                            ...
                            {filterTabs.map(tab => {
                                ...
                                return (
                                    <button key={tab.value} onClick={() => apply({ filter: tab.value, search })}
                                        style={{ padding: "8px 14px", fontSize: "9px", fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: "0.1em", border: `1px solid ${active ? "#0A0A0A" : "#e5e7eb"}`, backgroundColor: active ? "#0A0A0A" : "#fff", color: active ? "#fff" : "rgba(45,50,62,0.5)", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                                        {tab.label}
                                        <span style={{ marginLeft: "4px", fontSize: "8px", opacity: 0.6 }}>({tab.count})</span>
                                    </button>
                                );
                            })}
                        </div>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                        <input
                            ...
                            style={{ flex: 1, minWidth: 0, border: "1px solid #e5e7eb", padding: "8px 14px", fontSize: "12px", outline: "none", fontFamily: "inherit" }}
                        />
                        <button onClick={() => apply({ search, filter: filters.filter })}
                            style={{ padding: "8px 16px", backgroundColor: "#0A0A0A", color: "#fff", fontSize: "9px", fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: "0.1em", border: "none", cursor: "pointer", flexShrink: 0 }}>
                            Search
                        </button>
                        {(filters.search || filters.filter !== 'all') && (
                            <button onClick={() => { setSearch(''); apply({ filter: 'all', search: '' }); }}
                                style={{ padding: "8px 12px", fontSize: "9px", fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: "0.1em", border: "1px solid #e5e7eb", backgroundColor: "#fff", color: "rgba(45,50,62,0.4)", cursor: "pointer", flexShrink: 0 }}>
                                Clear
                            </button>
                        )}
                    </div>
                                        ...
                                        <div key={v.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 80px 80px 100px 110px 120px", gap: "16px", alignItems: "center", padding: "13px 24px", backgroundColor: "#fff" }}>
                                            ...
                                            <div>
                                                {isEditing ? (
                                                    ...
                                                    <span onClick={() => startEdit(v)} title="Click to edit stock"
                                                        style={{ display: "inline-block", padding: "3px 10px", fontSize: "11px", fontWeight: 900, fontVariantNumeric: "tabular-nums", backgroundColor: status.bg, color: status.color, border: `1px solid ${status.border}`, cursor: "pointer" }}>
